          ...
          <view v-for="(item,index) in seatArray" :key="index" 
            class="sm-cell" :style="'width:'+boxWidth+'px;height:'+seatSize+'px'">
             ...
             <template v-if="seat.flag === 1">
                <image v-if="seat.type===0" class="sm-icon" src="/static/unselected-l.png" mode="aspectFit"></image>
                <image v-else-if="seat.type===1" class="sm-icon" src="/static/selected-l.png" mode="aspectFit"></image>
                <image v-else-if="seat.type===2" class="sm-icon" src="/static/bought.png" mode="aspectFit"></image>
             </template>
             <!-- // 情侣右座 -->
             <template v-else-if="seat.flag === 2">
                <image v-if="seat.type===0" class="sm-icon" src="/static/unselected-r.png" mode="aspectFit"></image>
                <image v-else-if="seat.type===1" class="sm-icon" src="/static/selected-r.png" mode="aspectFit"></image>
                <image v-else-if="seat.type===2" class="sm-icon" src="/static/bought.png" mode="aspectFit"></image>
             </template>
             <!-- // 普通座位 其他座位 -->
             <template v-else>
                <image v-if="seat.type===0" class="sm-icon" src="/static/unselected.png" mode="aspectFit"></image>
                <image v-else-if="seat.type===1" class="sm-icon" src="/static/selected.png" mode="aspectFit"></image>
                <image v-else-if="seat.type===2" class="sm-icon" src="/static/bought.png" mode="aspectFit"></image>
             </template>
          	</view>
          </view>
